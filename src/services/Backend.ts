...
export interface BackendImpl {
  ...
  readonly bin: string
  readonly run: (prompt: string, opts: RunOptions) => Effect.Effect<number, OmniError, CommandExecutor>
  ...
  readonly passthrough: (argv: ReadonlyArray<string>) => Effect.Effect<number, OmniError, CommandExecutor>
}
